-- Healthcare scheduling / GP-at-hand registration SQL examples
-- Companion write-up: docs/DETAILED_SQL_WALKTHROUGH.md (Healthcare section)
-- Company names genericized as Healthcare Technology Company / PCAH
...
-- Q1b: share of eligible users who register for PCAH
SELECT
  SUM(CASE WHEN pcah_reg_date IS NOT NULL THEN 1 ELSE 0 END)
  / SUM(CASE WHEN pcah_eligible IS TRUE THEN 1 ELSE 0 END) AS prp_reg_oo_elig
FROM accounts a
LEFT JOIN pcah_reg g ON a.account_id = g.account_id;

-- Q1c: monthly PCAH registration rate and MoM change (2018)
SELECT
  date_trunc('month', account_created_date) AS date,
  SUM(CASE WHEN pcah_eligible = True AND pcah_reg_date IS NOT NULL THEN 1 ELSE 0 END)
    / SUM(CASE WHEN pcah_eligible = True THEN 1 ELSE 0 END) AS months_rate,
  100 * (
    SUM(CASE WHEN pcah_eligible = True AND pcah_reg_date IS NOT NULL THEN 1 ELSE 0 END)
      / SUM(CASE WHEN pcah_eligible = True THEN 1 ELSE 0 END)
    - LAG(
        SUM(CASE WHEN pcah_eligible = True AND pcah_reg_date IS NOT NULL THEN 1 ELSE 0 END)
          / SUM(CASE WHEN pcah_eligible = True THEN 1 ELSE 0 END)
...
-- Q1d: median latency account -> PCAH registration (2018 signups)
SELECT
  PERCENTILE_CONT(0.5) WITHIN GROUP (
    ORDER BY DATEDIFF(day, account_created_date, pcah_reg_date)
  ) AS median_latency
FROM accounts ac
LEFT JOIN pcah_reg gr ON ac.account_id = gr.account_id
