# Detailed SQL walkthrough


The following examples are SQL written to answer each prompt. Dialects vary by warehouse. Healthcare queries also live in [`../sql/healthcare_scheduling.sql`](../sql/healthcare_scheduling.sql). Related walkthroughs: [`sports_league_sql.md`](sports_league_sql.md), [`ecommerce_order_sql.md`](ecommerce_order_sql.md), [`mobile_game_marketing_analytics.md`](mobile_game_marketing_analytics.md).

## Streaming Video Platform

### Table name - “streams”: streaming data of Streaming Video Platform customers. It contains information at a stream level, with data such as minutes streamed, buffers encountered while streaming, location, device used, and the customer ID.
...
### Table name - "transactions": contains information on transactions conducted by customers on Streaming Video Platform. 
Example rows:  
...
Healthcare Technology Company allows users to book appointments with GPs via their phone. This also means that we need to actually schedule GPs to be available for those appointments. The appointments are scheduled in 10 minute blocks, but the GPs are scheduled for several hours at a time. Each GP can only see one patient at a time. Assume you have these 3 tables: slots, booked_gp_slots, and requested_appointment_slots.
...
### b) Why do you think Healthcare Technology Company would care about this analysis?

This would provide insights on utilization. Healthcare Technology Company would be able to see if they need more/less/or the same number of GPs to handle the number of patients at a certain time.
